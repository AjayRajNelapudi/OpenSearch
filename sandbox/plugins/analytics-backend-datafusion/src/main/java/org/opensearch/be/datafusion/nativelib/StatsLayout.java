 ...
 *
 * <p>The layout contains 7 named groups (2 runtime × 9 fields + 4 task monitor × 3 fields
 * + 1 partition semaphore × 4 fields = 34 longs = 272 bytes).
 */
public final class StatsLayout {
    ...
    private static final String[] PARTITION_SEMAPHORE_FIELDS = {
        "max_permits",
        "active_permits",
        "total_wait_duration_ms",
        "total_partitions_started" };
    ...
    public static final StructLayout LAYOUT = MemoryLayout.structLayout(
        ...
        partitionSemaphoreGroup("partition_semaphore")
    );
    ...
    // ---- VarHandles for partition_semaphore fields ----
    private static final VarHandle PS_MAX_PERMITS = handle("partition_semaphore", "max_permits");
    private static final VarHandle PS_ACTIVE_PERMITS = handle("partition_semaphore", "active_permits");
    private static final VarHandle PS_TOTAL_WAIT_DURATION_MS = handle("partition_semaphore", "total_wait_duration_ms");
    private static final VarHandle PS_TOTAL_PARTITIONS_STARTED = handle("partition_semaphore", "total_partitions_started");
    ...
    /**
     * Read the partition semaphore group (4 fields) from the segment.
     *
     * @param seg the memory segment containing the DfStatsBuffer
     * @return a populated PartitionSemaphoreStats instance
     */
    public static PartitionSemaphoreStats readPartitionSemaphore(MemorySegment seg) {
        return new PartitionSemaphoreStats(
            (long) PS_MAX_PERMITS.get(seg, 0L),
            (long) PS_ACTIVE_PERMITS.get(seg, 0L),
            (long) PS_TOTAL_WAIT_DURATION_MS.get(seg, 0L),
            (long) PS_TOTAL_PARTITIONS_STARTED.get(seg, 0L)
        );
    }
    ...
    private static StructLayout partitionSemaphoreGroup(String name) {
        return MemoryLayout.structLayout(
            ...
            ValueLayout.JAVA_LONG.withName("total_partitions_started")
        ).withName(name);
    }
